...
class TestModelExtensionSaleOrder(common.TransactionCase):
    def test_cron_archive_and_remove_selected(self):
        ...
        archived_orders = self.env[
            "sale.order.archive"
        ]._cron_archive_and_remove_selected()

        number_of_sale_orders_after_cron = self.env["sale.order"].search_count([])
        self.assertEqual(
            number_of_all_sale_orders,
            number_of_sale_orders_after_cron + archived_orders,
        )
        self.assertEqual(
            bool(self.env["sale.order"].search([("id", "=", so1_id)])), False
        )
        self.assertEqual(
            bool(self.env["sale.order"].search([("id", "=", so2_id)])), False
        )
